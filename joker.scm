...
(define (make-ordered-deck)
  ...
  (se (make-suit 'H) (make-suit 'S) (make-suit 'D) (make-suit 'C) '(0Z 0Z)))
...
(define (best-total cards)
  ...
  (define (count-jokers cards)
    (if (empty? cards) 0
      (if (equal? (value (first cards)) 0) (+ 1 (count-jokers (butfirst cards))) (count-jokers (butfirst cards)))))
  ...
  (define (choose-best-total total aces jokers)
    (if (= 0 (+ jokers aces)) total 
       (if (> aces 0)
        (if (>= 11 (+ total (+ aces jokers)))
           (choose-best-total (+ total 11) (- aces 1) jokers) 
           (choose-best-total (+ total 1) (- aces 1) jokers))
        (if (>= 11 (+ total (+ aces jokers)))
           (choose-best-total (+ total 11) aces (- jokers 1))
           (if (= jokers 1) (choose-best-total (+ total (- 21 total)) aces (- jokers 1))
               (choose-best-total (+ total 1) aces (- jokers 1)))))))
  (choose-best-total (total-without-aces cards) (count-aces cards) (count-jokers cards)))
  
(best-total '(ad 8s))    ; 19
(best-total '(ad 8s 5h)) ; 14
(best-total '(ad as 9h)) ; 21
(best-total '(0Z as 9h)) ; 21
(best-total '(0Z ad 8s)) ; 21
(best-total '(0Z 0Z ad))
